...
def main():
    ...
    apk_hash = digest(apk)
    smoke = json.loads((ROOT / 'build/ci/android-preview/runtime/smoke-result.json').read_text())
    require(smoke.get('passed') is True and smoke.get('apkSha256') == apk_hash, 'This exact APK did not pass preview smoke.')
...
The workflow passed its configured JVM tests/lint, verified the signature, alignment and both-mode renderer package, and installed/launched this exact APK and exercised Standard practice in an API36 emulator. Real-phone multiplayer, native 2D/3D interaction, accessibility and performance are still under testing. This is not a production release.
