...
The table uses an orthographic Camera3D in a real SubViewport, a bounded set of meshes, one light, and no shadows or postprocessing. Card faces are wordless original PartyDeck textures with independent upright rank labels. Shared rounded card geometry gives multisample antialiasing a real silhouette to smooth; generic face/back materials retain anisotropic mipmap filtering. The original 512×768 artwork has ample detail at phone card sizes. Transparent corner RGB preserves the card base color during filtering without changing visible artwork or increasing texture dimensions.

The 3D target follows the stage's physical display size with 4× MSAA, bounded to 2560 pixels per edge and 3 Mi pixels total. Its surface maps back to the logical stage, and camera projections use the same mapping for labels and touch targets. The camera fits the full table bounds with margin in both orientations. The center symbol uses alpha blending and anisotropic filtering to preserve its smooth edges.

Projected Control hit targets drive the shared controller; play never requires precision mesh picking or dragging. Selection uses a visible check and border with an optional 140 ms lift. Newly observed public plays move up to three face-down cards from the actor's side into the claim pile in under half a second. Only public counts/claim/turn data identify an accepted play; initial attachment, duplicate views and ambiguous backfill do not replay effects. Reduced Motion shows the settled state immediately. Mobile sound remains owned by the native shell; the standalone renderer uses the matching placement clip when sound is enabled. Background/close cancels feedback, and the 3D viewport returns to its retained idle frame after motion ends.

Short landscape uses a table plus a scrolling information panel. Compact or enlarged text keeps the hand toggle in the header and pins public context only when enough height remains for the scrollable table. In shorter windows, the round, rank, current actor and complete claim move into the scrolling content. Selection preserves scroll and keyboard focus, keeping the focused card inside the visible area after feedback reflows the layout. Reused raster card/rank assets, original vectors, licensed fonts and generated audio are inventoried in [assets/manifest.json](assets/manifest.json). Full engine/native notices are in [licenses/README.txt](licenses/README.txt); font notices are under `assets/licenses/`.
