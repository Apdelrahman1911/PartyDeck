...
func _render(next_state: Dictionary) -> bool:
	...
	var compact_cover := _text_scale >= 1.5 or _compact_phone_layout()
	_cover_art.visible = not compact_cover and size.x >= 420
	...
	_cover_description.visible = not compact_cover or hand.is_empty()
...
func _update_public_table(game: Dictionary, foreground: bool, closed: bool) -> void:
	...
	_surface.visible = not resolved and not closed and _text_scale < 1.5 and not _compact_phone_layout()
...
func _compact_phone_layout() -> bool:
	# Keep rank, claim and the whole Reveal control above the fixed phone actions.
	# Short landscape already puts the hand first; large text keeps its scroll layout.
	return size.x < 860 and size.y >= 500 and size.y < 800 and _text_scale < 1.5
...
func _relayout(refresh_cards: bool = true) -> void:
	...
	_hand_title.visible = not short_window and not _compact_phone_layout()
	...
	_cover_art.visible = _text_scale < 1.5 and not _compact_phone_layout() and size.x >= 420
