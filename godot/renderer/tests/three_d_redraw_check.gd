...
func _exercise_redraws() -> void:
	...
	# checking stable resize behavior, then exercise minimum size and zero view count.
	...
	var usable_view_count := _viewport.view_count
	_table._viewport_container.stretch = false
	# Pinned Viewport::_set_size clamps node sizes to 2 by 2; the actual server-only
	# zero-size draw gate was exercised above. Do not pretend this node can stay 0 by 0.
	_viewport.size = Vector2i.ZERO
	_check(_viewport.size == Vector2i(2, 2), "a node-level zero-size request uses the engine's 2 by 2 minimum")
	_draw("minimum-sized target", RenderingServer.VIEWPORT_UPDATE_ONCE)
	_draw("minimum-sized target idle", RenderingServer.VIEWPORT_UPDATE_DISABLED)
	_viewport.view_count = 0
	_draw("zero view count", RenderingServer.VIEWPORT_UPDATE_DISABLED, false)
	_viewport.view_count = usable_view_count
