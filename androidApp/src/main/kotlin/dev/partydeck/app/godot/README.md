...
`AndroidGodotPresentationHost` implements the common presentation factory seam. Its caller supplies the modes allowed by the packaged activation profile. The shipping profile lists both `2d` and `3d`, with Standard table selected initially. Shipping exposure requires recorded qualification. Creating a factory is inert; `open` captures the selected 2D/3D mode and launch preferences, requires shell-owned sound (`soundEnabled=false`), and encodes the existing recipient-safe `EngineLaunch` with `LastLightWireCodec`.
