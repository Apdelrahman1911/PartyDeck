...
| --- | --- | --- |
| Android/JVM TLS | [JSSE reference](https://docs.oracle.com/en/java/javase/21/security/java-secure-socket-extension-jsse-reference-guide.html), `SSLContext`, `SSLSocket`, `X509TrustManager` | A `ServerSocket` accepts TCP, then immediately wraps each accepted socket in a server-mode `SSLSocket`. No application bytes are exposed until TLS succeeds. Retaining the raw socket allows cancellation to abort blocked TLS I/O. Outgoing connections verify the complete leaf-certificate pin; both directions require TLS 1.2 or 1.3. |
| Certificate construction on Java | [Bouncy Castle 1.85 POM](https://repo.maven.apache.org/maven2/org/bouncycastle/bcpkix-jdk18on/1.85/bcpkix-jdk18on-1.85.pom), [bcprov 1.85.2 POM](https://repo.maven.apache.org/maven2/org/bouncycastle/bcprov-jdk18on/1.85.2/bcprov-jdk18on-1.85.2.pom), [official repository](https://github.com/bcgit/bc-java) | `bcpkix-jdk18on:1.85` and the updated `bcprov-jdk18on:1.85.2` are pinned explicitly. Use the X.509 builder with native JCA P-256 keys and signatures; do not install a global replacement security provider. |
...
| Apple Bonjour TXT | [listener service initializer](https://developer.apple.com/documentation/network/nwlistener/service-swift.struct/init(name:type:domain:txtrecord:)-8qh5), [NWTXTRecord dictionary](https://developer.apple.com/documentation/network/nwtxtrecord/dictionary) | The listener accepts a typed `NWTXTRecord`; the browser validates public version/name/port fields and preserves the native service endpoint for resolution. No pairing credentials appear in TXT. |
...
Manual endpoint information in the invitation bypasses multicast discovery. If a host address changes, public discovery may provide a candidate replacement, but the original fingerprint must still match. Android can retry a cached NSD result; iOS can retry the invitation's Bonjour service endpoint after an unreachable direct address. The retry retains the original overall handshake deadline, and an explicit authentication failure is terminal. If the host is recreated and its ephemeral certificate changes, guests need a new invitation. There is no certificate-warning bypass.

Java address enumeration prefers interfaces with an [IPv4 broadcast address](https://docs.oracle.com/en/java/javase/21/docs/api/java.base/java/net/InterfaceAddress.html#getBroadcast()) before other active routes, then IPv4 before IPv6 within each interface. This reduces the chance that an unrelated mobile or VPN address becomes the first direct invitation endpoint. It retains other usable addresses, including IPv6-only interfaces, and does not assume platform interface names or query Android network state. Actual multi-interface reachability remains part of device qualification.
...
Every accepted or outgoing native connection owns an absolute ten-second handshake watchdog. Java's [`Socket.setSoTimeout`](https://docs.oracle.com/en/java/javase/21/docs/api/java.base/java/net/Socket.html#setSoTimeout(int)) only bounds an individual blocking read; a peer that continually trickles partial TLS data must not retain a slot by making occasional progress. Readiness, timeout expiry and cleanup have ownership guards so a stale timer cannot close an already-ready connection. The shared adapter separately caps queued native chunks per connection so one overflowing peer cannot terminate other guests or the listener.
...
On 2026-09-09, Linux validation passed all eighteen transport JVM tests and Android transport compilation. Six use actual JSSE sockets for pinned maximum-size ordered frames, changed-pin rejection, discovery fallback with the original pin, invalid/truncated frame rejection, an actively trickling handshake deadline, and cancellation of blocked TLS I/O. Ten common/security tests cover codec boundaries, canonical invitation parsing, per-peer flood isolation and prompt-cancellation ownership; two validate the JVM side of the mixed-implementation fixture. Separate controller integration tests use three real TLS transports and exercise private card delivery and reconnection to the same seat.

[CI run 34378549932](https://github.com/Apdelrahman1911/PartyDeck/actions/runs/34378549932) also passed all ten common transport tests on the iOS simulator and linked both device and simulator frameworks. Generated `PartyDeckKit.h` confirms the Swift driver/observer methods, factory/adapter initializers, payload/port types and failure-code names used by the native networking sources. The app-hosted Swift TLS tests, mixed JVM/Swift execution and physical-device qualification are tracked separately in `docs/release-qualification.md`; shared-framework linking does not establish native TLS interoperability.
