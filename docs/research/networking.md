...
On 2026-09-09, `:transport:jvmTest :transport:compileAndroidMain` passed all fifteen transport tests. Five use actual JSSE sockets for pinned maximum-size ordered frames, changed-pin rejection, invalid/truncated frame rejection, an actively trickling handshake deadline, and cancellation of blocked TLS I/O. Ten common/security tests cover codec boundaries, canonical invitation parsing, per-peer flood isolation and prompt-cancellation ownership. Separate controller integration tests use three real TLS transports and exercise private card delivery and reconnection to the same seat. Native iOS test execution and cross-platform physical-device qualification are tracked separately in `docs/release-qualification.md`.
