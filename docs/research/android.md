...
At commit `c73a659221f94a5ba7eeafa38a4b7a0754a54265`, [run 34384326819, attempt 2](https://github.com/Apdelrahman1911/PartyDeck/actions/runs/34384326819/attempts/2) retained both variant reports from an API-36, 1080 × 2400 px emulator:

- **Debug:** readiness rejected a System UI ANR before app installation. No debug app flow passed in this attempt. Full logcat identifies `com.android.systemui/.SystemUIService` exceeding its service-start deadline by waiting 20,064 ms. Guest CPU pressure was 88.93% over the preceding ten seconds, with total CPU usage at 99–100%; guest memory pressure was 0.21%, with the full-stall average at 0.00%. The host had 6,615 MiB available before emulator startup, no Java process in the retained process snapshot, and zero swap use before/after. This evidence supports first-boot CPU saturation and does not support a leftover Kotlin daemon or host-memory explanation. The separate `lastanr` dump reported no retained ANR; the full logcat supplied the service ANR evidence.
- **Optimized, test-signed:** the installed APK (`2c4adc0151dbcc2330507d3761eebc4732a699037557a0db0a73ed8fc615efb9`) passed rules, persistence of all three switches after process restart, reveal/select/hide, selection clearing and concealment after background/restore, an accepted one-card play, leaving practice, local host creation, QR/copy/Android Sharesheet cancellation, host teardown, and invalid-invitation editing with name retention. At 200% text, Home actions, rules, settings, and invalid-invitation feedback were reached. Twenty flow screenshots and the final diagnostic screenshot were retained. The full variant still **failed** during the subsequent invitation replacement, before the 200% private-hand/action checks.

The failed replacement's screenshot and XML show a focused editable field containing `not-an-invitedited-invitationtion`: the script typed into text that its unverified Select all/Delete sequence had not cleared. The corrected helper requires the tagged editable field to hold focus, inspects the result of Select all/Delete, uses bounded backward/forward deletion if text remains, and requires exact empty text before typing. Final replacement equality remains strict. Syntax/CLI checks and 167 input/real-XML replay cases passed, including ignored Select all at every cursor position, wrapped text, refusal to type without focus or a successful clear, and rejection of an incorrect final value. These are helper checks; the correction still requires execution in Android CI. Existing readiness logic is extracted as `prepare_device()` for separately recorded emulator preparation without changing the per-variant ANR rejection.
...
The retained attempt-2 screenshots did not show a soft keyboard. The updated API-36 smoke saves/enables/restores the secure `show_ime_with_hard_keyboard` setting, which Android 16's `InputMethodService` explicitly observes and honors. It requires a focused nonsecret Join-name field and `mInputShown`, `mIsInputViewShown`, `mDecorViewVisible`, and `mWindowVisible` to all be true before retaining `join-name-soft-ime` and `large-text-join-name-soft-ime` screenshots and redacted IME dumps, then dismisses the keyboard normally. This closes the harness coverage gap; actual default/200% IME layout qualification still awaits execution of the updated CI script. The setting is an Android implementation detail used only by the test harness, not an application API. [Secure setting source](https://android.googlesource.com/platform/frameworks/base/+/refs/tags/android-16.0.0_r1/core/java/android/provider/Settings.java), [IME setting and visibility source](https://android.googlesource.com/platform/frameworks/base/+/refs/tags/android-16.0.0_r1/core/java/android/inputmethodservice/InputMethodService.java)
