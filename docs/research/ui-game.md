...
Research checked on 2026-09-09. Owner: `ui_game`. Initial deliverable is design research only; implementation begins after the coordinator's environment gate and interface freeze. The rule source of truth is [Last Light rules](../game-rules.md), not this presentation document.
...
- A normal-width phone can show five small cards together. Narrow windows use a horizontally scrollable rail with a visible continuation, or a two-row arrangement when the available height makes that clearer. Large text uses a readable grid/list and taller faces. Do not shrink text or targets to preserve the five-card row.
