...
- The implemented emulator smoke installs the real APK and exercises Settings, Practice, leave confirmation, and return home through actual accessibility nodes. It requires KVM, uses a bounded API 36 AVD, and keeps ready-screen captures and failure diagnostics. It cannot prove several physical Android/iOS devices interoperate over a local network.
...
- Build the actual Swift application for `generic/platform=iOS` with `-sdk iphoneos` and signing disabled as well; a device Kotlin framework alone does not compile Swift code behind device-only compilation conditions.
