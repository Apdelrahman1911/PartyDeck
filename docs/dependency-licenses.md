...
Audit date: 2026-09-09. This audit covers resolved dependency inputs, published POM license declarations, embedded/source notices, and packaged resources. The optimized iOS app at `987d380` was also inspected after linking; DNG retention is confirmed below. The input inventory retains components even when final linking or Android shrinking removes some of their code.
...
DNG's [source-code agreement](../assets/licenses/runtime/skia_dng_sdk_LICENSE.source_code) grants redistribution rights, but section 5 addresses indemnification when distributing the software in a commercial product, and section 6 restricts trademark use. **The optimized iOS executable retains DNG code.** The release owner must account for these terms when distributing it commercially; including notices alone does not discharge the separate commercial obligation.

The final device link map from [run 34398824935](https://github.com/Apdelrahman1911/PartyDeck/actions/runs/34398824935), revision `987d380967149d80d8ba5761e206ecdf7386af25`, identifies 59 DNG object files and **1,635 live symbols across 58 objects**, separately from 1,720 dead-stripped symbols. Twelve live symbol addresses were independently matched to the actual device executable, whose SHA-256 is `9df831ae5350bd48e6165f31bc22d31b12b0cc9e085d89581fd5565729b4a283`. The complete DNG texts and acknowledgment match source in both packaged apps. The simulator link map covers only Xcode's small launcher stub; it is not evidence of DNG exclusion from the debug dylib.

## Resolved Swift packages and native boundary

The [Xcode Package.resolved](../iosApp/PartyDeck.xcodeproj/project.xcworkspace/xcshareddata/swiftpm/Package.resolved), originally verified from run `34378549932` and unchanged in successful run `34398824935`, has SHA-256 `47eb4a3e1acc5072f207aaf2d4dc924544afeeda3e01800079c81d05322b08d3`. The workspace file matches the captured CI artifact byte-for-byte. It selects exactly these packages:
...
The successful iOS job and actual optimized bundle now establish compilation, linking, packaged notices, and DNG retention. Its executable is unsigned and contains no test bundle or inspected interop-fixture names. Final signed-store archive validation remains separate. System frameworks referenced by the app, including Security, Network, CryptoKit, and Foundation, remain operating-system dependencies; they are not bundled third-party source packages. The simulator test archive contains XCTest support, which is absent from the optimized device app.
...
Independent package inspection verified all **65 legal resource files** byte-for-byte in the Android packages and both actual iOS app bundles, including the complete aggregate. The optimized Android artifacts at `987d380` match the previously audited package contents except for their recorded Git revision. The iOS device bundle also includes the app privacy manifest and five upstream Swift Crypto manifests. Exact archive hashes and inspection limits are recorded in [release qualification](release-qualification.md); independent native records are under `/tmp/partydeck-release-qa/final-987d380/ios/`.

These checks establish packaged notice coverage for the inspected artifacts, not a substitute for the publisher's commercial-license assessment or final store validation. After dependency upgrades, repeat Gradle and Xcode resolution, compare locked source revisions, update native/source evidence and texts, run `python3 assets/tools/bundle_notices.py`, verify hashes, and inspect rebuilt packages before updating release evidence.
