...
func _public_info(parent: VBoxContainer, compact: bool) -> void:
	...
	if game.phase == "PLAYING":
		...
		info.add_child(_label(turn, 14, false, CITRON if game.viewerId == game.turnPlayerId else MUTED))
		...
		info.add_child(_label(claim_text, 14, false, MUTED))
		if not compact and _state.textScale < 1.3:
			_roster(info)
...
func _roster(parent: VBoxContainer) -> void:
	...
	for player in _state.game.players:
		...
		seat.add_child(_label(_name(player.id), 12, false, color))
		seat.add_child(_label("Out · watching" if player.eliminated else "%d cards · fuse %d/6" % [int(player.handCount), int(player.penaltyAttempts)], 11, false, MUTED))
...
func _hand_controls(parent: VBoxContainer, large: bool) -> void:
	...
	if game.phase == "PLAYING":
		...
		if viewer == null or viewer.eliminated:
			...
			hand_header.add_child(_label("Your hand · %d cards" % game.yourHand.size(), 14, false, PAPER))
	...
	if not _state.status.is_empty():
		pane.add_child(_label(_state.status, 14, false, MUTED))
...
func _add_card(card_data: Dictionary, index: int, count: int, face_up: bool, private_card: bool, selected: bool,
	...
	if not face_up:
		return
	var label := _label(_rank(rank), 14, false, INK)
	...
	_card_bindings.append({"card": card, "label": label, "target": target, "explanation": explanation})
...
func _position_targets() -> void:
	...
	_camera.size = maxf(8.7, 4.7 * _stage.size.x / _stage.size.y)
	for binding in _card_bindings:
		...
		var label_point := _camera.unproject_position(binding.card.to_global(Vector3(0, 0.04, 0.52)))
		binding.label.position = Vector2(bounds.position.x, label_point.y - 7 * _state.textScale)
		...
		if binding.explanation != null:
			binding.explanation.position = Vector2(bounds.position.x - 16, bounds.end.y + 6)
			binding.explanation.size = Vector2(bounds.size.x + 32, 42 * _state.textScale)
