"""Audit every attempted entry, retaining the two late native Back failures."""
...
host_fields = ['enginePid', 'presentationId', 'displayDensity', 'readyAccepted', 'foregroundFrameReady',
    ...
    'diagnosticsRequested', 'diagnosticsTimedOut']
cases = []
for mode in ['2d', '3d']:
    for scale in ['1.0', '2.0']:
        ...
        for kind in entry_names:
            ...
            if teardown:
                reason = {'match': 'return_to_chooser', 'scene-exit': 'renderer_exit', 'native-exit': 'native_close'}[kind]
                ...
                assert mode == '3d' and kind == 'native-exit' and not result['passed']
                ...
                assert final['lifecycle'] == 'process_exit_requested' and final['closeReason'] == 'renderer_exit'
                ...
                assert not final['diagnosticsTimedOut']
                assert final['acceptedIntents'] == final['receivedIntents'] == final['authorityRejections'] == final['rejectedEvents'] == '0'
                fresh = read(directory / '01-fresh-concealed.json')
                assert fresh['revision'] == final['revision'] == '0'
                assert fresh['receivedEvents'] == '1' and final['receivedEvents'] == '2'
                assert fresh['diagnostics']['handConcealed'] and fresh['diagnostics']['selectedCount'] == 0
                assert fresh['diagnostics']['privateFaceCount'] == fresh['diagnostics']['privateLabelCount'] == 0
                ...
                chooser_resumes = [{'line': number, 'text': line} for number, line in enumerate(event_lines, 1)
                    if back_events[0]['line'] < number and re.search(r'\s' + str(result['device']['chooser_pid'])
                ...
                for launch in ['launch_2d', 'launch_3d']:
                    ...
                    assert len(nodes) == 1 and nodes[0].get('enabled') == 'true' and nodes[0].get('clickable') == 'true'
                entry['nativeBackFailure'] = {'expectedRoute': 'native_back', 'observedRoute': final['closeReason'],
                    'observedReceivedEventsDelta': int(final['receivedEvents']) - int(fresh['receivedEvents']),
                    'expectedNativeBackReceivedEventsDelta': 0,
                    'systemBackEvents': back_events, 'sameChooserPidResumeEvents': chooser_resumes,
                    'finalChooserControlsXmlSha256': digest(runtime / 'last-ui.xml'),
                    'dedicatedTeardownReceiptPresent': False,
                    'limits': 'Exact route check failed before the dedicated old-PID/chooser success loop; actual OS death and failure PID observations support shutdown but do not turn the requested route into a pass.'}
        ...
        assert len({entry['enginePid'] for entry in entries}) == len({entry['presentationId'] for entry in entries}) == 3
...
summary['scopeLimits'] = ['API 35 debug Android local-practice comparison only; no optimized runtime, API 36, physical-device, LAN, accessibility, audio dormancy, or store qualification.',
    ...
    'Both 3D native Back entries fail the expected native_back route because the retained host reports renderer_exit. Their actual destruction and chooser resume do not qualify the requested route.',
    'All twelve process logs retain an eglCodecCommon removeVertexArrayObject ERROR during shutdown; ten also retain the known shader-cache warning pair at error log priority. No clean-driver-log claim is made.',
