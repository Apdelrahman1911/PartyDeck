...
RUN_ID = 34460468965
HEAD = "31d148f9b6bbfa15d1746e1029858b86714362a3"
ATTEMPT = 1
...
contract = json.loads((BASE / "collection-contract.json").read_text())
assert contract["runId"] == RUN_ID and contract["expectedHeadSha"] == HEAD and contract["expectedAttempt"] == ATTEMPT
exclusive_json(BASE / "collection-resumption-1.json", {
    "runId": RUN_ID, "headSha": HEAD, "attempt": ATTEMPT, "resumedAtUtc": utc(),
    "reason": "Initial local exec session ended with exit 143 while remote workflow remained in progress. Cause is unestablished; existing verified evidence is retained unchanged.",
    "originalContractSha256": hashlib.sha256((BASE / "collection-contract.json").read_bytes()).hexdigest(),
})
downloaded = {name for name in EXPECTED if (BASE / (name + ".downloaded.json")).is_file()
              and (BASE / (name + ".integrity.json")).is_file()}
for name in downloaded:
    receipt = json.loads((BASE / (name + ".integrity.json")).read_text())
    assert receipt["zipDigestMatchesApi"] and receipt["zipSizeMatchesApi"] and receipt["headSha"] == HEAD
saved_logs = set()
for receipt_path in BASE.glob("job-*.collected.json"):
    receipt = json.loads(receipt_path.read_text())
    path = Path(receipt["path"])
    assert path.is_file() and receipt["headSha"] == HEAD
    assert hashlib.sha256(path.read_bytes()).hexdigest() == receipt["sha256"]
    saved_logs.add(receipt["jobId"])
permanent_errors = {}
pending = {}
attempts = {}
previous = {}
