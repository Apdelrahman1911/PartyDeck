...
input_audit = read(base / 'input-evidence-summary.json')
...
old = read(Path('/tmp/partydeck-engine-ci/34434392993/authority-package-evidence-summary.json'))
...
package_summary = {'auditUsesExactRunSource': True, 'runId': int(run), 'headSha': head, 'scope': 'Actual linked Simulator app identity; runtime qualification is separate.',
    ...
    'plist': plist, 'packSha256': pack_sha, 'inputAuditReceipt': receipt(base / 'input-evidence-summary.json'),
    ...
    'comparisonWithFirstAuthorityRun': {'runId': 34434392993, 'previousPackageSummarySha256': digest(Path('/tmp/partydeck-engine-ci/34434392993/authority-package-evidence-summary.json')),
...
(base / 'authority-package-evidence-summary.json').write_text(json.dumps(package_summary, indent=2) + '\n')
print(json.dumps({'packageAuditPassed': True, 'appBytes': archive.stat().st_size, 'appSha256': digest(archive),
    'sameAppMemberPayloads': len(same_files), 'totalAppMembers': len(files), 'sameExecutable': package_summary['comparisonWithFirstAuthorityRun']['sameExecutable']}), flush=True)
