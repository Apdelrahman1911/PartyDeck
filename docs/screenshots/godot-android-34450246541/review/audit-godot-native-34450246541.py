"""Audit the next exact run, including strict native Back event locality."""
    ...
    for scale in ['1.0', '2.0']:
        ...
        for kind in entry_names:
            ...
            if teardown:
                ...
                assert mode == '3d' and scale == '1.0' and kind == 'match' and not result['passed']
                ...
                entry['closeAcknowledgmentFailure'] = {'expectedRoute': 'return_to_chooser', 'observedRoute': final['closeReason'],
                    ...
                    'limits': 'The strict close-acknowledgment check failed before the dedicated old-PID/chooser success loop. Actual OS death and final chooser observations do not qualify the missing acknowledgment. The native Back correction was not exercised in this case.'}
...
summary['scopeLimits'] = ['API 35 debug Android local-practice comparison only; no optimized runtime, API 36, physical-device, LAN, accessibility, audio dormancy, or store qualification.',
    ...
    'The 3D normal case fails a missing closeSignalAcknowledged marker during match return and never attempts the fresh scene-Exit or native-Back entries. Its actual process death does not qualify that marker. The 3D 200% case separately verifies native_back with unchanged received-event count.',
