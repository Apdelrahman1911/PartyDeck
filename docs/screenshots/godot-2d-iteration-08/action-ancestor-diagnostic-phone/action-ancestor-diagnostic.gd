...
func _run() -> void:
	...
	if state.game.get("phase") == "PLAYING" and not state.game.get("yourHand", []).is_empty():
		if not await _check_action_drag():
			return
...
func _check_action_drag() -> bool:
	var scroll: ScrollContainer = _main.find_child("TableScroll", true, false)
	var target: Button = get_first_node_in_group("partydeck_action_reveal")
	...
	while ancestor != null:
		if ancestor is Control:
			hierarchy.append({"node": str(ancestor.get_path()), "filter": ancestor.mouse_filter, "class": ancestor.get_class()})
			var ancestor_name := str(ancestor.get_path())
			ancestor.gui_input.connect(func(event: InputEvent) -> void:
				event_log.append({"target": ancestor_name, "event": event.as_text(), "scroll": scroll.scroll_vertical})
			)
		ancestor = ancestor.get_parent()
	print(JSON.stringify({"ancestors": hierarchy, "targetFilter": target.mouse_filter}))
	scroll.gui_input.connect(func(event: InputEvent) -> void:
		event_log.append({"target": "TableScroll", "event": event.as_text(), "scroll": scroll.scroll_vertical})
	)
	target.gui_input.connect(func(event: InputEvent) -> void:
		event_log.append({"target": "RevealButton", "event": event.as_text(), "scroll": scroll.scroll_vertical})
	)
	var hover := InputEventMouseMotion.new()
	hover.position = start
	hover.global_position = start
	...
	for step in range(1, 5):
		var motion := InputEventMouseMotion.new()
		motion.position = start + Vector2(0, -20 * step)
		motion.global_position = motion.position
		...
		await process_frame
	var up := InputEventMouseButton.new()
	up.position = start + Vector2(0, -80)
	up.global_position = up.position
	up.button_index = MOUSE_BUTTON_LEFT
	up.pressed = false
	Input.parse_input_event(up)
	await _settle()
	var result := {"before": before, "after": scroll.scroll_vertical, "handVisible": _main.controller.presentation_state().handVisible, "start": str(start), "max": scroll.get_v_scroll_bar().max_value, "page": scroll.get_v_scroll_bar().page, "events": event_log, "ancestors": hierarchy, "targetFilter": target.mouse_filter}
	FileAccess.open(_output.path_join("action-gesture-diagnostics.json"), FileAccess.WRITE).store_string(JSON.stringify(result, "\t"))
	print(JSON.stringify(result))
	if scroll.scroll_vertical <= before or _main.controller.presentation_state().handVisible:
		_fail("Dragging on Reveal did not scroll the page without activating Reveal.")
		return false
	return true
