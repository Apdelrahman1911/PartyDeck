...
These are actual Godot-rendered images. Iteration 01 uses synthetic structural data; iteration 02 uses frozen projections generated by the real Kotlin authority. The early 3D v1–v3 captures also use authority-derived static data and retain their camera/layout failures. These static renderer checks do not establish authority-accepted gameplay or native mobile accessibility. Originals from every supplied iteration are retained.
