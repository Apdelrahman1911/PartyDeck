...
Use the latest native and shared-UI collections for current review. Earlier captures and the first Godot structural iteration are retained below for comparison. A screenshot shows a particular state; the run and fixture limits remain part of its evidence.
...
These are actual Godot-rendered images. Iteration 01 uses synthetic structural design data; it does not establish game-authority integration or mobile accessibility. Later 2D/3D comparison captures can be added as separate iterations without replacing these originals.
...
- Godot iteration 01 contains only synthetic recipient-safe structural data. Its report and fixture provenance are retained with the captures. No LAN or admission credentials are involved.
