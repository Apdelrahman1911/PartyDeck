"""Audit exact run 34463877910 against its four observed passing native case receipts."""
...
assert run == '34463877910' and head == 'bbfde024d95f5e4d62a26b57495a6a90447d6033'
...
assert correlation['headSha'] == head and correlation['sourceArchiveReceipt']['receipt']['headSha'] == head
assert correlation['nativeWorkflowJobUnchanged'] and correlation['strictTeardownCheckUnchanged']
assert correlation['closeBoundsUnchanged'] and correlation['closeBoundsMs'] == {'closeFallbackMs': 250, 'rendererExitWaitMs': 1500}
...
for mode in ['2d', '3d']:
    for scale in ['1.0', '2.0']:
        ...
        assert result['passed'] is True, (mode, scale, result.get('stage'), result.get('error'))
        ...
        assert entry_names == ['match', 'scene-exit', 'native-exit']
        ...
        for kind in entry_names:
            ...
            assert teardown is not None, (mode, scale, kind, 'Missing successful teardown receipt')
            final = teardown['host']
            ...
            expected_stages = ['requested', 'dispatch started', 'native barrier', 'main callback']
            ...
            entry['closeSignalLogObservation'] = {
                ...
                'requestToMainCallbackMs': elapsed['main callback'] - elapsed['requested'],
...
assert [case['passed'] for case in cases] == [True, True, True, True]
assert summary['nativeOriginalPngsDecoded'] == 80
...
assert len(all_entries) == 12 and all(entry['completedRequestedExitRoute'] for entry in all_entries)
...
summary['scopeLimits'] = ['API 35 debug Android local-practice comparison only; no optimized runtime, API 36, physical-device, LAN, accessibility, audio dormancy, or store qualification.',
    ...
    'Original artifact ZIP integrity is recorded separately in the collector integrity receipts; no visual approval is claimed by this automated audit.']
timings = [entry['closeSignalLogObservation'] for entry in all_entries]
assert all(row['nativeBarrierObserved'] and row['mainCallbackObserved'] and not row['fallbackObserved'] for row in timings)
assert all(0 <= row['requestToNativeBarrierMs'] <= row['requestToMainCallbackMs'] < 250 for row in timings)
destroy_ms = [int(entry['finalHost']['nativeDestroyElapsedMs']) for entry in all_entries]
assert all(0 <= elapsed < 1500 for elapsed in destroy_ms)
summary['closeTiming'] = {'routes': len(all_entries), 'nativeBarriersObserved': len(timings),
    'mainCallbacksObserved': len(timings), 'fallbacksObserved': 0,
    'sourceBoundsMs': correlation['closeBoundsMs'],
    'requestToNativeBarrierMs': {'min': min(row['requestToNativeBarrierMs'] for row in timings), 'max': max(row['requestToNativeBarrierMs'] for row in timings)},
    'requestToMainCallbackMs': {'min': min(row['requestToMainCallbackMs'] for row in timings), 'max': max(row['requestToMainCallbackMs'] for row in timings)},
    'nativeDestroyElapsedMs': {'min': min(destroy_ms), 'max': max(destroy_ms)},
    'scope': 'Observed exact-PID stages and recorded native destruction; unchanged source bounds, without an inferred scheduling cause.'}
summary['scope'] = 'Read-only archived-source, package, native result/log, and capture-integrity audit for the four API 35 debug comparison cases. Independent visual/accessibility review remains separate.'
