...
terminal_path = build / 'ci/terminal-return/results/report.json'
terminal = read(terminal_path)
assert terminal['result'] == 'passed' and terminal['checks']
assert all(check['passed'] is True for check in terminal['checks'])
terminal_log_path = build / 'ci/terminal-return/runtime.log'
terminal_log = terminal_log_path.read_text()
assert not re.search(r'(?m)^(?:SCRIPT ERROR|ERROR|Parse Error):', terminal_log)
summary['packedTerminalReturn'] = {'passed': True, 'checks': len(terminal['checks']),
    'scope': terminal['limitations'], 'renderingMethod': terminal['renderingMethod'],
    'reportPath': str(terminal_path), 'reportSha256': digest(terminal_path),
    'runtimeLogPath': str(terminal_log_path), 'runtimeLogSha256': digest(terminal_log_path)}
