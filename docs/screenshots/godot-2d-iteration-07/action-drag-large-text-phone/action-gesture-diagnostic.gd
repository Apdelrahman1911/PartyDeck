...
func _check_action_drag() -> bool:
	...
	Input.parse_input_event(up)
	await _settle()
	var result := {"before": before, "after": scroll.scroll_vertical, "handVisible": _main.controller.presentation_state().handVisible, "start": str(start), "max": scroll.get_v_scroll_bar().max_value, "page": scroll.get_v_scroll_bar().page, "events": event_log}
