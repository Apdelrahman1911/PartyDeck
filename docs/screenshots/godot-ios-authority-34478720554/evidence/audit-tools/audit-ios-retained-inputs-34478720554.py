...
check('completed producer audit at exact revision', producer['runId'] == 34478720554 and producer['headSha'] == HEAD and all(c['passed'] for c in producer['checks']))
run = read(BASE / 'last-run-status.json')
jobs = read(BASE / 'last-jobs-status.json')['jobs']
check('exact completed run/head/event/attempt', run['id'] == 34478720554 and run['head_sha'] == HEAD and run['event'] == 'workflow_dispatch' and run['run_attempt'] == 1 and run['status'] == 'completed')
context = read(RETAINED / 'evidence/retained-ci-context.json')
check('retained CI exact revision, workflow revision, stage, run and architecture', context['source_commit'] == context['expected_source_commit'] == context['workflow_sha'] == HEAD and context['requested_stage'] == 'test' and context['run_id'] == '34478720554' and context['run_attempt'] == '1' and context['system'] == 'Darwin' and context['machine'] == 'arm64')
workflow = (SOURCE / '.github/workflows/godot-ios-authority-host.yml').read_text()
consumer_jobs = [job for job in jobs if job['id'] in (102876625127, 102876625041)]
