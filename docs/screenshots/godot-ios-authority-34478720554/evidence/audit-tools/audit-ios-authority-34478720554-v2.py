...
old = read(Path('/tmp/partydeck-engine-ci/34473325295/authority-package-evidence-summary.json'))
...
package_summary = {'auditUsesExactRunSource': True, 'runId': int(run), 'headSha': head, 'scope': 'Actual linked Simulator app identity; runtime qualification is separate.',
    ...
    'comparisonWithPreviousAuthorityRun': {'runId': 34473325295, 'previousPackageSummarySha256': digest(Path('/tmp/partydeck-engine-ci/34473325295/authority-package-evidence-summary.json')),
...
package_path = base / 'authority-package-evidence-summary.json'
if package_path.exists():
    assert read(package_path) == package_summary, 'Existing successful package audit must remain identical.'
else:
...
issues = [dict(row) for row in con.execute('SELECT rowid AS issueRowId,uuid,testCaseRun_fk,compactDescription,detailedDescription,issueType FROM TestIssues')]
...
for test_manifest in manifest:
    ...
    attachments, measurements, actual_geometry, observed_controls, background_states, rejected_observations = [], [], [], [], [], []
    # Xcode's launch-failure issue-description attachment has no export timestamp.
    # Preserve that absence; placing it after timed siblings supplies no event time.
    for item in sorted(test_manifest['attachments'], key=lambda item: (item.get('timestamp') is None, item.get('timestamp') or 0)):
        path = attachments_path / item['exportedFileName']
        value_sha = digest(path)
        record = {**receipt(path), 'name': item['suggestedHumanReadableName'], 'timestamp': item.get('timestamp'),
            'exportTimestampPresent': 'timestamp' in item,
            'testIdentifier': identifier, 'exportedFileUuid': path.stem}
        if path.stem in rows:
            ...
            record.update(attachmentUuid=row['uuid'], xcresultPayloadRef=row['xcResultKitPayloadRefId'],
                xcresultPayloadSha256=obj['decodedSha256'], xcresultOriginalObject=obj['source'],
                matchesOriginalXcresultPayload=True, verificationKind='xcresult_payload')
        else:
            # xcresulttool generates this export directly from the issue text.
            # It has no attachment row or payload object, so verify the exact
            # original TestIssues bytes without inventing either identity.
            assert path.suffix == '.txt' and record['name'] == 'Complete Issue Description.txt'
            assert case['result'] == 'Failure' and not record['exportTimestampPresent']
            matching = [(issue, column) for issue in issues if issue['testCaseRun_fk'] == case['runRow']
                for column in ['compactDescription', 'detailedDescription']
                if isinstance(issue[column], str) and path.read_bytes() == issue[column].encode('utf-8')]
            assert len(matching) == 1, 'Issue export must match exactly one original description.'
            issue, column = matching[0]
            record.update(matchesOriginalXcresultPayload=False, matchesOriginalXcresultIssueText=True,
                verificationKind='xcresult_issue_description', xcresultIssueRowId=issue['issueRowId'],
                xcresultIssueUuid=issue['uuid'], xcresultIssueColumn=column,
                xcresultDatabase=str(bundle / 'database.sqlite3'),
                note='Original CI export matches issue text exactly; no attachment-row payload or export timestamp exists.')
        if path.suffix == '.png':
            picture = read_png(path.read_bytes())
            record.update(width=picture.width, height=picture.height, pngVerifiedAndDecoded=True)
        if path.suffix == '.json':
            value = read(path)
            if {'timeoutSeconds', 'elapsedSeconds', 'measurements'} <= set(value):
                rejected_observations.append({'name': record['name'], 'file': str(path), 'timestamp': record['timestamp'], 'value': value})
                record['jsonKind'] = 'last_rejected_wait_observation'
            elif 'measurements' in record['name'].lower():
    ...
    per_test.append({**case, 'issues': [issue for issue in issues if issue['testCaseRun_fk'] == case['runRow']],
        ...
        'observedControls': observed_controls, 'backgroundStates': background_states,
        'lastRejectedWaitObservations': rejected_observations, 'independentChecks': checks})
...
summary = {'runId': int(run), 'headSha': head, 'manifestSha256': digest(manifest_path), 'sqliteSha256': digest(bundle / 'database.sqlite3'),
    ...
    'xcresultHashMatchedAttachments': sum(item['matchesOriginalXcresultPayload'] for item in records),
    'xcresultIssueTextMatchedExports': sum(item.get('matchesOriginalXcresultIssueText', False) for item in records),
    'testResults': dict(Counter(test['result'] for test in tests)),
    'lastRejectedWaitObservationCount': sum(len(test['lastRejectedWaitObservations']) for test in per_test),
    'attachmentsWithoutExportTimestamp': [item['path'] for item in records if not item['exportTimestampPresent']],
    'tests': per_test, 'attachments': records, 'scope': 'Original attachment payload identity and independently checked measured state/geometry. Actual Home is established by recorded dock accessibility and SpringBoard state; applicationReportedBackground is preserved as observed, with no invented state. Last rejected observations are kept separate from later measurements. Missing export timestamps remain null; placement after timed siblings supplies no event time. Visual review and failure diagnosis are separate.'}
