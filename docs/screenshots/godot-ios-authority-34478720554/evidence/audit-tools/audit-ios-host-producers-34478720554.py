...
check('exact maintained patch order; optional mouse patch absent', patches == ['coreaudio-dormancy', 'main-loop-access'])
...
freeze_path = BASE / 'expected-native-freeze-v6.json'
freeze = read(freeze_path)
check('supplied native freeze v6 identity', digest(freeze_path) == '7273afd091d9cca95396e061758635400af5705f2a10f01c0a693c6783c110a0')
for group in ['files', 'renderer_sources']:
    for name, expected in freeze[group].items():
        check('archived source matches supplied freeze: ' + name, digest(SOURCE / name) == expected)
for name in freeze['deleted_files']:
    check('supplied deleted source remains absent: ' + name, not (SOURCE / name).exists())
check('producer native sources equal supplied module snapshot', engine['native_module_sources'] == freeze['module_snapshot']['native_module_sources'] and module_digest == freeze['module_snapshot']['module_snapshot_sha256'])
check('producer upstream patches equal supplied freeze', engine['upstream_patches'] == freeze['upstream_patches'])
check('PCK identity, length and entry count equal supplied freeze', pack['pack']['sha256'] == freeze['qualified_pack_candidate_sha256'] and pack['pack']['bytes'] == freeze['pack_bytes'] and len(pack['pack']['entries']) == freeze['pack_entries'])
expected = read(BASE / 'coordinator-expected-inputs.json')
check('coordinator expected exact run revision', expected['runId'] == int(RUN) and expected['headSha'] == HEAD)
check('coordinator runtime source identity', digest(HOST / 'modules/partydeck_ios_probe/PDGodotRuntime.mm') == expected['runtimeSourceSha256'])
check('coordinator retained test source identity', digest(HOST / 'RetainedHostUITests/RetainedHostUITests.swift') == expected['retainedTestSourceSha256'])
check('coordinator module snapshot identity', module_digest == expected['moduleSnapshotSha256'])
summary = {
    ...
    'expectedNativeFreeze': record(freeze_path), 'primarySourceReferences': record(BASE / 'research/references.json'),
    'optionalStartupMousePatchIncluded': False, 'threeDDemandOptimizationExpected': True,
