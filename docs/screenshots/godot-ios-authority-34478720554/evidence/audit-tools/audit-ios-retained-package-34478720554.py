...
package_summary={'runId':34478720554,'headSha':HEAD,'scope':'Linked app package and static symbol identity only. No app was executed during this audit.','archive':record(archive),'fileCount':len(files),'files':files,'executable':{'bytes':len(exe),'sha256':sha(exe),'machOUuid':uuid,'cpuType':cpu,'fileType':filetype,'buildVersions':versions,'symbolTableEntries':nsyms,'requiredDefinedSymbols':matched_symbols,'matchingDebugRecordsExcludedFromDefinitionCheck':debug_symbols},'plist':plist,'packSha256':sha(pack),'inputAudit':record(BASE/'retained-input-evidence-summary.json'),'originalSymbolLog':record(RETAINED/'retained-host-test/evidence/symbols.log'),'checks':package_checks,'binaryFormatReferences':record(BASE/'research/macho/references.json'),'symbolDefinitionAuditMethod':'Exclude N_STAB debugging entries before counting N_SECT definitions, following the preserved primary Apple headers.'}
