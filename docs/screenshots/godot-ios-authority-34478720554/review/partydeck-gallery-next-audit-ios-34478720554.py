...
EXPECTED = {
    ...
    '34478720554': {'revision':'c6ea1dd9f7966517fbee87a06b633d01c432c24d',
        'pack':'d6adbba1b5cbd6a1ac3a5754e4294363eae70ae9540af4ba12040cd4cfff44e0',
        'freeze':'e154d73f2f5b91571b107b9d9c81ebc2372563f11ac8546c0112bb558814e3f5',
        'pngs':43},
}[RUN]
...
def independently_check_measurement_groups(case, suite):
    check_results = []
    for group in ['measurements','homeObservations','dormantIntervals','touchCandidates','lastRejectedWaitObservations']:
    ...
    for record in case.get('lastRejectedWaitObservations',[]):
        value = record['value']
        check_results.append({'original_last_rejected_wait':record['file'],
            'timeout_seconds':value['timeoutSeconds'],'elapsed_seconds':value['elapsedSeconds'],
            'exact_rejected_measurements_preserved_separately_from_later_capture':True})
...
def audit_suite(suite):
    ...
    issue_columns = 'rowid AS issueRowId,uuid,' if suite == 'authority' else ''
    issues = [dict(row) for row in con.execute('SELECT ' + issue_columns + 'testCaseRun_fk,compactDescription,detailedDescription,issueType FROM TestIssues')]
    ...
    for raw in manifest:
        ...
        for item in sorted(raw['attachments'],key=lambda x:x.get('timestamp',float('inf'))):
            path = attachments_dir / item['exportedFileName']
            op = owner_by_path[str(path)]
            assert op['testIdentifier'] == identifier and op['timestamp'] == item.get('timestamp')
            assert op['name'] == item['suggestedHumanReadableName']
            if op.get('matchesOriginalXcresultIssueText'):
                issue = next(x for x in issues if x['issueRowId'] == op['xcresultIssueRowId'])
                assert issue['uuid'] == op['xcresultIssueUuid'] and issue['testCaseRun_fk'] == case['runRow']
                assert path.read_bytes() == issue[op['xcresultIssueColumn']].encode('utf-8')
                assert sha(path) == op['sha256'] and path.stat().st_size == op['bytes']
                assert 'timestamp' not in item and op['timestamp'] is None
                attachments.append(original(path,ci_run=RUN,source_revision=REV,source_revision_exact=True,
                    host_suite=suite,test_identifier=identifier,test_name=case['name'],test_result=case['result'],
                    timestamp=None,suggested_human_readable_name=item['suggestedHumanReadableName'],
                    attachment={**item,'testIdentifier':identifier},relative_path='attachments/'+path.name,
                    original_xcresult_payload_independently_verified=False,
                    original_xcresult_issue_text_independently_verified=True,
                    xcresult_issue_row_id=issue['issueRowId'],xcresult_issue_uuid=issue['uuid'],
                    xcresult_issue_column=op['xcresultIssueColumn'],
                    note='Original issue-text export has no attachment payload row or export timestamp; neither is invented.'))
        ...
        cases.append({**{k:case[k] for k in keep if k in case},'capture_files':[Path(x['source_original']).name for x in case_captures],
            ...
            'last_rejected_wait_files':[x['file'] for x in case.get('lastRejectedWaitObservations',[])]})
    ...
    return {'suite':suite,'owner_audit':original(owner_path),'manifest':original(attachments_dir/'manifest.json'),
        ...
        'original_payloads_independently_verified':len(captures)+len(attachments)-issue_export_count,
        'original_issue_text_exports_independently_verified':issue_export_count,'skipped_tests':skipped,
