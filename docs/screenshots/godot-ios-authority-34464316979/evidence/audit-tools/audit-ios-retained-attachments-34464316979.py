...
DECODED = Path('/root/projects/PartyDeck/artifacts/evidence-storage/34464316979-retained-xcresult-decoded')
...
expected={'testActiveAndDormantBackgroundTransitionsStayConcealed()':'Success','testRepeated2DAnd3DKeepOneDormantEngine()':'Success','testStaleFirstReadyAndCloseCompletionReentry()':'Failure'}
check('exact three case results in original XCResult',len(tests)==3 and {t['name']:t['result'] for t in tests}==expected)
...
check('required suite qualification remains false',result['retained_engine_lifecycle_qualified'] is False and result['same_process_reentry_qualified'] is False and result['command_exit_code']==65)
...
for tm in manifest:
    ...
    measurements=[];targets=[];intervals=[];homes=[];case_records=[]
    ...
    run_ids={m['value']['runID'] for m in measurements}
    check(identifier+' one application journal run ID',len(run_ids)==1)
    run_id=next(iter(run_ids));journal_path=HOST/'evidence/application-observations'/('retained-runtime-'+run_id+'.json');journal=read(journal_path)
    ...
    if case['name']=='testRepeated2DAnd3DKeepOneDormantEngine()':
    ...
    elif case['name']=='testActiveAndDormantBackgroundTransitionsStayConcealed()':
        ...
        check(identifier+' failure contains second 3D entry only',len(measurements)==1 and measurements[0]['value']['entryCount']==2 and measurements[0]['value']['mode']=='3d' and measurements[0]['value']['state']=='running')
        failed=measurements[0]['value']
        check(identifier+' old-handle probe not observed',failed['staleProbe']=={} and not targets and len(journal['journal'])==2)
        probe=journal['journal'][0]['measurements']['firstReadyProbe']
        check(identifier+' initial stale Ready rejection observed in journal',probe['attempted'] and probe['completed'] and not probe['accepted'] and not probe['nativeReadyConfirmedAtRejection'] and int(probe['currentGeneration'])>int(probe['capturedGeneration']) and probe['reportedGenerationBeforeCompletion']==probe['currentGeneration'])
        closure(failed['lastClosure'],identifier+' observed first close')
        check(identifier+' first close precedes switched presentation',failed['lastClosure']['reason']=='SWITCH_FROM_CLOSE_COMPLETION' and failed['lastClosure']['presentationID']!=failed['presentationID'])
        validated=['initial stale Ready rejection and first close are recorded observations only','case remains failed while waiting for current diagnostics for second 3D entry','old-handle probe and subsequent play/exit were not reached']
    per_test.append({**case,'issues':[i for i in issues if i['testCaseRun_fk']==case['runRow']],'attachmentCount':len(case_records),'byExtension':dict(Counter(Path(r['path']).suffix for r in case_records)),'runID':run_id,'applicationJournal':record(journal_path),'journalArguments':journal['arguments'],'measurements':measurements,'touchCandidates':targets,'touchCandidatesByAction':dict(Counter(t['value']['action'] for t in targets)),'eligibleVisibleTargetCount':sum(t['eligibleVisibleTarget'] for t in targets),'dormantIntervals':intervals,'homeObservations':homes,'independentlyVerifiedScope':validated})
    print(json.dumps({'test':identifier,'result':case['result'],'attachments':len(case_records),'measurements':len(measurements),'touchCandidates':len(targets),'dormantIntervals':len(intervals)}),flush=True)
check('exact manifest case inventory',{t['identifier'] for t in tests}=={m['testIdentifier'] for m in manifest})
check('all 94 original attachment payloads verified',len(records)==94)
summary={'runId':34464316979,'headSha':HEAD,'attempt':1,'scope':'Original attachment identity and independently verified bounded Simulator observations. The third case remains failed and the full retained suite remains unqualified. Touch-candidate attachments precede the last input eligibility check and are not counted as actual taps. Home evidence preserves raw applicationState even when it reports foreground alongside observed SpringBoard dock icons.','manifest':record(ATTACHMENTS/'manifest.json'),'sqlite':record(BUNDLE/'database.sqlite3'),'decodedObjectIndex':record(DECODED/'decoded-object-index.json'),'originalResult':record(HOST/'evidence/result.json'),'inputAudit':record(BASE/'retained-input-evidence-summary.json'),'packageAudit':record(BASE/'retained-package-evidence-summary.json'),'attachmentCount':len(records),'xcresultHashMatchedAttachments':len(records),'pngCount':sum(Path(r['path']).suffix=='.png' for r in records),'measurementCount':sum(len(t['measurements']) for t in per_test),'touchCandidateCount':sum(len(t['touchCandidates']) for t in per_test),'dormantIntervalCount':sum(len(t['dormantIntervals']) for t in per_test),'testResults':dict(Counter(t['result'] for t in tests)),'tests':per_test,'attachments':records,'checks':checks,'retainedEngineLifecycleQualified':False,'sameProcessReentryQualified':False,'physicalSensorsQualified':False,'deviceMetalQualified':False,'actualAudioInterruptionQualified':False,'limitations':['Intermediate dormant samples are represented by the recorded sample count and endpoint measurements; all original XCResult and journals remain available.','Passing XCTest pause assertions are not independently reconstructed from AX logs in this attachment audit.','A failed case is not promoted by selected successful observed substeps.','Pixel/interaction review and source failure diagnosis are separate owner supplements.']}
