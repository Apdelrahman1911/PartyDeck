...
internal class SessionQualificationObservation {
    ...
    fun snapshot(state: AppUiState, generation: Long, projection: Pair<Long, Long>?): String {
        ...
        return """{"supported":true,"exhausted":$exhausted,"sessionGeneration":"$generation","sessionPresent":${session != null},"practice":${state.isPractice},"screen":"${state.screen.name}","sessionRevision":${counter(session?.revision)},"phase":${quoted(game?.phase)},"round":${game?.roundNumber ?: 0},"ownTurn":${game != null && game.viewerId != null && game.turnPlayerId == game.viewerId},"handCount":${game?.yourHand?.size ?: 0},"canSendAction":${state.canSendSessionAction},"canPlay":${actions?.canPlay == true},"canAdvanceRound":${session?.controls?.canAdvanceRound == true},"pending":${quoted(state.pendingAction)},"problem":${quoted(state.problem?.code)},"privacyEpoch":"${state.privacyEpoch}","leaveConfirmation":${state.leaveConfirmationRequested},"foreground":${state.isForeground},"backgrounded":${state.isBackgrounded},"mode":"${state.presentation.selected.name}","lifecycle":"${state.presentation.lifecycle.name}","presentationOrdinal":"${if (state.presentation.presentationId != null) ordinal else 0L}","projectedRendererRevision":${counter(projection?.first)},"projectedSessionRevision":${counter(projection?.second)},"lastViewerReceipt":$lastReceipt}"""
    }
}
